    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[radial-gradient(1200px_600px_at_50%_-10%,rgba(212,165,58,.07),transparent_60%)]">
      <div className="w-full max-w-sm bg-panel border border-line rounded-[20px] p-8 shadow-2xl animate-fade-in">
        <div className="flex items-center gap-2.5 justify-center font-extrabold text-[19px] mb-1.5">
          <span className="w-[34px] h-[34px] rounded-[10px] bg-gradient-to-br from-accent to-accent-2 flex items-center justify-center text-black font-black text-[19px] shadow-lg shadow-accent/20">F</span>
          Finance OS
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button disabled={loading} className="w-full py-3 bg-gold hover:bg-gold-2 text-black font-bold rounded-[12px] text-[14.5px] transition-colors disabled:opacity-50 mt-1">{loading ? 'Creando…' : 'Crear cuenta'}</button>
        </form>
