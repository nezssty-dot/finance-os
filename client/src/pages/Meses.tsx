...
function printTicket(movs: any[], month: number, year: number) {
  ...
  const incRows = movs.filter((m) => m.type === 'INCOME')
    .map((m) => `<div class="r"><span>${escapeHtml(m.description)}</span><b>${ARS(Number(m.amount))}</b></div>`).join('')
  const expRows = movs.filter((m) => m.type === 'EXPENSE')
    .map((m) => `<div class="r"><span>${escapeHtml(m.description)}</span><b>${ARS(Number(m.amount))}</b></div>`).join('')

  const html = `<!DOCTYPE html><html><head><title>Ticket ${MONTHS[month]} ${year}</title><style>@page{size:80mm auto;margin:8mm}*{margin:0;padding:0;box-sizing:border-box;font-family:'SF Mono',Consolas,monospace;font-size:13px}body{background:#fff;color:#1a1a17;padding:20px}.c{text-align:center}.b{font-family:sans-serif;font-weight:800;font-size:18px;letter-spacing:.14em}.t{font-size:10px;letter-spacing:.3em;color:#55524a;margin-top:5px;text-transform:uppercase}.m{font-family:sans-serif;font-size:20px;font-weight:700;margin-top:12px}.d{border-top:1.5px dashed #c9c4b6;margin:14px 0}.s{font-family:sans-serif;font-size:10px;font-weight:700;letter-spacing:.12em;color:#8a8578;text-transform:uppercase;margin:4px 0 8px}.r{display:flex;justify-content:space-between;padding:2px 0}.r b{font-weight:600}.big{display:flex;justify-content:space-between;font-family:sans-serif;font-size:16px;font-weight:800;padding:7px 0}.pos{color:#2f8f52}.neg{color:#c0463f}.f{text-align:center;font-size:9px;color:#a8a294;margin-top:14px;font-family:sans-serif}</style></head><body><div class="c"><div class="b">FINANCE<span style="color:#2ee88f">.</span>OS</div><div class="t">Ticket mensual</div><div class="m">${MONTHS[month]} ${year}</div></div><hr class="d"><div class="s">Ingresos</div>${incRows}<div class="r"><b>Total</b><b class="pos">${ARS(inc)}</b></div><hr class="d"><div class="s">Egresos</div>${expRows}<div class="r"><b>Total</b><b class="neg">${ARS(exp)}</b></div><hr class="d"><div class="big"><span>Restante</span><span class="${inc - exp >= 0 ? 'pos' : 'neg'}">${ARS(inc - exp)}</span></div><div class="f">Finance OS · resumen del mes</div></body></html>`

  // Iframe oculto en vez de window.open: Electron bloquea los pop-ups, y por eso el
  // "Exportar PDF" de Meses no hacía nada. Con el iframe no hay ventana que bloquear.
  ...
  const run = () => {
    ...
    finally { setTimeout(() => { if (frame.parentNode) document.body.removeChild(frame) }, 1000) }
  }
