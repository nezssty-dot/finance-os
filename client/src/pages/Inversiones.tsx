...
import { WealthCard, Card, Button, Modal, Input, Select, EmptyState, AsyncGate } from '@/components/ui'
import { useToast } from '@/lib/toast'

// Cada tipo con su etiqueta legible. El valor (izquierda) es lo que entiende el backend;
// la etiqueta (derecha) es lo que ve el usuario. Así "STOCK" se muestra como "Acción".
...
const KINDS = ['BTC', 'ETH', 'USDT', 'STOCK', 'FUND', 'FIXED_TERM', 'USD', 'PESOS'] as const
...
export function Inversiones() {
  ...
  const [updating, setUpdating] = useState<InvItem | null>(null)
  ...
  async function handleSave(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast('Inversión agregada', 'success')
    } catch (e: any) { toast(e.message || 'No se pudo guardar', 'error') }
    setSaving(false)
  }
    ...
    <>
      ...
      <div className="p-7 animate-fade-in">
        ...
        <Card>
            ...
            return (
              <div key={i.id} className="flex items-center gap-3 py-3 border-b border-bg-2 last:border-0">
                ...
                <div className="flex flex-col gap-1 shrink-0 ml-1">
                  <button
                    onClick={() => { setUpdating(i); setNewValue(String(i.currentValue)) }}
                    className="text-[11px] text-accent hover:underline"
                    title="Actualizar cuánto vale hoy"
                  >
                    Actualizar
                  </button>
                  <button
                    onClick={async () => {
                      if (!confirm(`¿Eliminar ${i.name}? Esta acción no se puede deshacer.`)) return
                      try {
                        await api(`/investments/${i.id}`, { method: 'DELETE' })
                    ...
                    className="text-[11px] text-txt-3 hover:text-danger"
          ...
          }) : <EmptyState icon="📈" title="Sin inversiones" description="Registrá tu primera inversión para seguir tu portafolio." action={<Button variant="primary" onClick={() => setOpen(true)}>Agregar primera inversión</Button>} />}
        </Card>
      </div>
      ...
      <Modal open={!!updating} onClose={() => setUpdating(null)} title="Actualizar valor" sub={updating?.name}>
        <p className="text-[12.5px] text-txt-3 mb-4">
          Poné cuánto vale hoy esta inversión. La ganancia se recalcula sola contra tu capital
          de {updating ? money(updating.capital, updating.currency) : ''}.
        </p>
        <Input
          label="Valor actual"
          type="number"
          ...
          autoFocus
        />
        {updating && Number(newValue) > 0 && (
          <div className="text-[12px] mt-2">
            {(() => {
              const g = Number(newValue) - updating.capital
              const pct = updating.capital > 0 ? ((g / updating.capital) * 100).toFixed(1) : '0'
              return (
                <span className={g >= 0 ? 'text-success' : 'text-danger'}>
                  {g >= 0 ? 'Ganancia' : 'Pérdida'}: {g >= 0 ? '+' : ''}{money(g, updating.currency)} ({pct}%)
                </span>
              )
            })()}
          </div>
        ...
        <div className="flex gap-3 mt-5">
          <Button type="button" className="flex-1" onClick={() => setUpdating(null)}>Cancelar</Button>
          <Button
            variant="primary"
            className="flex-1"
            disabled={saving || !(Number(newValue) > 0)}
            onClick={async () => {
              if (!updating) return
              setSaving(true)
              try {
                await api(`/investments/${updating.id}`, { method: 'PATCH', body: { currentValue: Number(newValue) } })
                toast('Valor actualizado', 'success')
                setUpdating(null)
                refetch()
              } catch (err) {
                // Si algo falla, el usuario lo ve en vez de que "no pase nada".
