...
import { WealthCard, Card, MovementRow, InteractiveDonut, EmptyState, AsyncGate, Button, Modal, Input, Select } from '@/components/ui'
    ...
    <>
      ...
      <div className="p-7 animate-fade-in">
        <div className="flex justify-end mb-4">
          <Button variant="primary" onClick={() => setQuickAdd(true)}>+ Nuevo Movimiento</Button>
        </div>
        ...
        {(() => {
          ...
          const wealthCards = [
            ...
            { key: 'inv', label: 'Invertido', value: p.invertido, sub: 'En inversiones', accent: 'violet' as const },
            usdInvertido !== 0 && { key: 'invUsd', label: 'Invertido USD', value: usdInvertido, currency: 'USD', sub: 'Inversiones en dólares', accent: 'blue' as const },
            { key: 'deu', label: 'Deudas', value: p.deudas, sub: 'Total a pagar', accent: 'danger' as const },
            { key: 'cob', label: 'Por cobrar', value: p.porCobrar, sub: 'Pendiente', accent: 'warning' as const },
          ].filter(Boolean) as Array<{ key: string; label: string; value: number; hero?: boolean; sub?: string; currency?: string; tone?: 'auto' | 'neutral'; accent?: 'green' | 'blue' | 'violet' | 'danger' | 'warning' }>
        ...
        {dash.categories.length > 0 && (
          <Card className="mt-4">
            ...
            <InteractiveDonut
              size={150}
              slices={dash.categories.map((c: any) => ({ name: c.name, value: c.amount, color: c.color }))}
            />
          </Card>
        )}
      </div>

      <Modal open={quickAdd} onClose={() => setQuickAdd(false)} title="Nuevo Movimiento" sub="Sumá un ingreso o gasto en segundos">
        <form
          onSubmit={async (e) => {
            e.preventDefault()
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            <Select name="type" label="Tipo" defaultValue="EXPENSE">
