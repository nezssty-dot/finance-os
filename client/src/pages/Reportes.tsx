...
import { Card, WealthCard, Button, InteractiveDonut, EmptyState, SkeletonCard } from '@/components/ui'
...
export function Reportes() {
  ...
  function exportPDF() {
    ...
    const paleta = ['#2ee88f', '#5b9bd4', '#22c977', '#d9615c', '#b06fd4', '#d4915b', '#4fb3a8', '#c96b9a', '#8a8a8a']
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  header { display: flex; justify-content: space-between; align-items: flex-end;
           border-bottom: 2px solid #2ee88f; padding-bottom: 12px; margin-bottom: 20px; }
  .brand { font-size: 20px; font-weight: 800; letter-spacing: -0.4px; }
  .brand span { color: #2ee88f; }
  ...
  .card.hero { border-color: #2ee88f; background: #f0fdf6; }
            ...
            <div className="grid lg:grid-cols-2 gap-4">
              <Card title="Gasto por categoría">
                {!data.categories.length ? (
                  ...
                  <div className="mt-2">
                    <InteractiveDonut
                      slices={data.categories.map((c: any) => ({ name: c.name, value: c.amount, color: c.color }))}
                    />
                  </div>
                )}
              </Card>
