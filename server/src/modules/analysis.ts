...
analysisRouter.get(
  ...
  ah(async (req, res) => {
    ...
    // Monthly totals
    const [movements, fx] = await Promise.all([
    ...
    // Convierte a pesos. Un servicio en USD (ej: Claude US$20) debe pesar su equivalente en
    // pesos según el dólar del día, NO 20 pesos. Sin cotización, ese movimiento no se suma
    // (se prefiere un total honesto antes que uno con dólares contados como pesos).
    ...
    for (const m of movements) {
      ...
      if (amt === null) continue; // sin cotización para esa moneda: no se suma
